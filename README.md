...
```bash
curl -fsSL https://ametel.dev/agentreceipt/install.sh | sh -s -- --version v0.10.1
```
...
```bash
curl -fsSL https://ametel.dev/agentreceipt/install.sh | sh -s -- --version v0.10.1 --install-skill
```
...
```bash
curl -fsSL https://ametel.dev/agentreceipt/install.sh | sh -s -- --version v0.10.1 --no-install-skill
```
...
AGENTRECEIPT_INSTALL_SKILL=1 AGENTRECEIPT_INSTALL_VERSION=0.10.1 AGENTRECEIPT_INSTALL_DIR="$HOME/.local/bin" sh scripts/install.sh
```
